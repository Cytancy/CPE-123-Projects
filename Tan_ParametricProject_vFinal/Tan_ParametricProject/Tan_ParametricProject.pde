...
//Parametric Project | Optimization v4.3
...
void setup() {
  ...
  player = minim.loadFile("BGM3.MP3");
...
void reset()
{
  sfxplayer = minim.loadFile("SFX2.MP3");
  sfxplayer.play();
  player.close();
  player = minim.loadFile("BGM3.MP3");
  score=0;
...
void gameStart(){
  if (startOffset<=450) {
    ...
    noStroke();
    gameStartRing++;
    if (gameActive) {startOffset+=5; lives=3; timer = 10; initiateMusic();}
    musicOn=true;
  }
}
//=====================================================
...
void runTimer()
{
  ...
  timer-=.02;
  noFill();
  if (timer<=0 && lives>0) {timer=10; lives--; sfxplayer = minim.loadFile("SFX3.MP3");sfxplayer.play(); lostLife =true; lostLifeCounter=20; }
}
//=====================================================
void gameOver()
{
  if (gameOverOffset==-450) {sfxplayer = minim.loadFile("SFX4.MP3"); sfxplayer.play();}
  fill (50);
...
//=====================================================
void mousePressed(){
  if (shape1Y == 218 && shapes2Offset[correctShape]>170 && shapes2Offset[correctShape]<320 && lives>0){println ("great success!");shape1Exiting=true; successful=true; combo++; timer+=(2+(1000/(score+100))); sfxplayer = minim.loadFile("SFX.MP3");sfxplayer.play();playSFX=false;}
  else if (gameActive && lives>0)  {lives--; combo=0; lostLife =true; lostLifeCounter=15; sfxplayer = minim.loadFile("SFX3.MP3");sfxplayer.play();}
